...
var waitFlag = false; //false meaning no wait needed-game can continue/start
var firstCardWord;
var firstCardId;
var secondCardId;
var secondCardWord;
...
function updateProgressBar(progress){
  for(let i=0;i<10;i++){
    if(progress === i ){
      var progressWidth = (i)*(11.11); //if total div (inside div) is 100% then we need to fill to 100% so 9*11.1111 = 100%
      var width = progressWidth+"%";
      var progressString = i + " / 9 Complete";
      document.getElementById('gameProgress').style.width=width;
      document.getElementById('progressSpan').textContent=progressString;
      document.getElementById('gameProgress').style.background = "green";
...
function updateScore(){
  ...
  if(score===9){ /*if all cards are matched*/
    var popUpElement =
    setTimeout(function(){
      gameEndOpenPopUp()
    },1000);
...
function gameEndOpenPopUp(){
  $('#popUpBox').css('display','block');
}
function closePopUp(){
  $('#popUpBox').css('display','none');
  shuffleCardsAssignNew();
}
